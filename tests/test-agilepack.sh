...
INPUT=data/agile-in.yml

# Conversion routine
CONVERT=./convert/agile2json.py
...
OUTPUT=data/agile-out.json

# Tell the tester what we're doing
echo " ================ Testing Agilepack Converter ====================="
...
(
    ...
    # for example:
    #
    # wget yourname.web.cern.ch/yourname/net.tgz
    # tar xf input.tar
    #
)

# intermediate file name (make sure it's in the temp dir)
JSON_FILE=$TMPDIR/agile.json

# run the conversion
echo "Running conversion $CONVERT $INPUT"
$CONVERT $INPUT > $JSON_FILE
